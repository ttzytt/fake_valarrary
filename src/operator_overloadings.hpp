...
Op_node<T>& operator+(Mask_array<T, SZ>& msk_array, Exp_tree<T>& sub_tr) {
    return *(new Op_node<T>(*(new Unknow_node<T>(msk_array.id)),
                            Op_node<T>::ADD, sub_tr));
}

template <typename T, size_t SZ>
Op_node<T>& operator+(Mask_array<T, SZ>& msk_array, T val) {
    return *(new Op_node<T>(*(new Unknow_node<T>(msk_array.id)),
                            Op_node<T>::ADD, *(new Const_node<T>(val))));
}

template <typename T, size_t SZ>
Op_node<T>& operator-(Mask_array<T, SZ>& msk_array, Exp_tree<T>& sub_tr) {
    return *(new Op_node<T>(*(new Unknow_node<T>(msk_array.id)),
                            Op_node<T>::SUB, sub_tr));
}

template <typename T, size_t SZ>
Op_node<T>& operator-(Mask_array<T, SZ>& msk_array, T val) {
    return *(new Op_node<T>(*(new Unknow_node<T>(msk_array.id)),
                            Op_node<T>::SUB, *(new Const_node<T>(val))));
}

template <typename T, size_t SZ>
Op_node<T>& operator*(Mask_array<T, SZ>& msk_array, Exp_tree<T>& sub_tr) {
    return *(new Op_node<T>(*(new Unknow_node<T>(msk_array.id)),
                            Op_node<T>::MUL, sub_tr));
}

template <typename T, size_t SZ>
Op_node<T>& operator*(Mask_array<T, SZ>& msk_array, T val) {
    return *(new Op_node<T>(*(new Unknow_node<T>(msk_array.id)),
                            Op_node<T>::MUL, *(new Const_node<T>(val))));
}

template <typename T, size_t SZ>
Op_node<T>& operator/(Mask_array<T, SZ>& msk_array, Exp_tree<T>& sub_tr) {
    return *(new Op_node<T>(*(new Unknow_node<T>(msk_array.id)),
                            Op_node<T>::DIV, sub_tr));
}

template <typename T, size_t SZ>
Op_node<T>& operator/(Mask_array<T, SZ>& msk_array, T val) {
    return *(new Op_node<T>(*(new Unknow_node<T>(msk_array.id)),
                            Op_node<T>::DIV, *(new Const_node<T>(val))));
}

template <typename T, size_t SZ>
Op_node<T>& operator%(Mask_array<T, SZ>& msk_array, Exp_tree<T>& sub_tr) {
    return *(new Op_node<T>(*(new Unknow_node<T>(msk_array.id)),
                            Op_node<T>::MOD, sub_tr));
}

template <typename T, size_t SZ>
Op_node<T>& operator%(Mask_array<T, SZ>& msk_array, T val) {
    return *(new Op_node<T>(*(new Unknow_node<T>(msk_array.id)),
                            Op_node<T>::MOD, *(new Const_node<T>(val))));
}

//----------------条件运算符----------------

template <typename T, size_t SZ>
Cond_node<T>& operator==(Mask_array<T, SZ>& msk_array, Exp_tree<T>& sub_tr) {
    return *(new Cond_node<T>(*(new Unknow_node<T>(msk_array.id)),
                              Cond_node<T>::EQ, sub_tr));
}

template <typename T, size_t SZ>
Cond_node<T>& operator==(Mask_array<T, SZ>& msk_array, T val) {
    return *(new Cond_node<T>(*(new Unknow_node<T>(msk_array.id)),
...
Exp_tree<uint64_t>& operator""_cons(uint64_t x) {
    return *new Const_node<uint64_t>(x);
}
